## Normal Map Explanation Conversation: 

[View conversation](https://claude.ai/share/ecad7dbb-b3a9-43ae-8e38-305d1fab48a4)
...
This (and almost all normal maps you find online) will have a blue-ish tint. This is because the normals are all closely pointing outwards towards the positive z-axis (0,0,1)
: a blue-ish color. The deviations in color represent normal vectors that are slightly offset from the general positive z direction, giving a sense of depth to the texture. For example, you can see that at the top of each brick the color tends to be more greenish, which makes sense as the top side of a brick would have normals pointing more in the positive y direction (0,1,0)
which happens to be the color green!
...
From the image we can see that the texture coordinate differences of an edge E2
of a triangle (denoted as ΔU2 and ΔV2) are expressed in the same direction as the tangent vector T and bitangent vector B. Because of this we can write both displayed edges E1 and E2 of the triangle as a linear combination of the tangent vector T and the bitangent vector B
:
E1=ΔU1T+ΔV1B
E2=ΔU2T+ΔV2B
...
We can calculate E
as the difference vector between two triangle positions, and ΔU and ΔV as their texture coordinate differences. We're then left with two unknowns (tangent T and bitangent B) and two equations. You may remember from your algebra classes that this allows us to solve for T and B
.
...
Try to visualize the matrix multiplications in your head and confirm that this is indeed the same equation. An advantage of rewriting the equations in matrix form is that solving for T
and B is easier to understand. If we multiply both sides of the equations by the inverse of the ΔUΔV
matrix we get:
[ΔU1ΔU2ΔV1ΔV2]−1[E1xE2xE1yE2yE1zE2z]=[TxBxTyByTzBz]
This allows us to solve for T
and B
. This does require us to calculate the inverse of the delta texture coordinate matrix. I won't go into the mathematical details of calculating a matrix' inverse, but it roughly translates to 1 over the determinant of the matrix, multiplied by its adjugate matrix:
[TxBxTyByTzBz]=1ΔU1ΔV2−ΔU2ΔV1[ΔV2−ΔU2−ΔV1ΔU1][E1xE2xE1yE2yE1zE2z]
This final equation gives us a formula for calculating the tangent vector T
and bitangent vector B
from a triangle's two edges and its texture coordinates.
...
This is a tutorial page from LearnOpenGL about Normal Mapping, a technique used in 3D graphics to add visual detail to surfaces without increasing the geometric complexity of the model.
The Core Problem

3D models are made of flat triangles. Even with color textures applied, the surfaces look unrealistically flat under lighting because the lighting system only sees one normal vector per surface, not the fine details like cracks, holes, or bumps that real surfaces have.
The Solution: Normal Mapping

Instead of using a single normal vector for an entire surface, normal mapping uses a normal map texture — a special image where each pixel stores a normal vector (direction) rather than a color. This tricks the lighting algorithm into thinking the surface has far more geometric detail than it actually does, creating the illusion of bumps, dents, and texture without adding more triangles to the model.
Key Technical Concepts Explained

Storing normals as colors: Since normal vectors have x, y, z components (ranging from -1 to 1), they can be encoded as RGB colors (0 to 1). This is why normal maps typically have a blue-ish tint — blue represents the z-axis, which points outward from the surface.

Tangent Space: A normal map is created in a local coordinate system relative to the surface it's on. The challenge is that the same normal map won't work correctly if you rotate or reorient the surface. The solution is "tangent space" — a coordinate system local to each surface that allows the normal map to work regardless of the surface's orientation in world space.

TBN Matrix: To transform between coordinate spaces, you need three perpendicular vectors: Tangent, Bitangent, and Normal. These form the TBN matrix that converts normal map data from tangent space into world space (or vice versa).
Why It Matters

Normal mapping provides massive visual improvement (making flat surfaces look detailed) with minimal performance cost compared to adding actual geometry. It's one of the most important techniques in modern 3D graphics for creating realistic-looking scenes efficiently.

so in my lighting shader I just have to use the normals from the normal map as the surface normal and then transform it the world space?
18:30

Essentially yes, but with an important caveat: you need to transform it correctly using the TBN matrix.

Here's the basic flow:

    Sample the normal map in your fragment shader to get a normal vector in tangent space
    Transform it to world space by multiplying it with the TBN matrix
    Use that transformed normal in your lighting calculations instead of the interpolated surface normal

The code looks like this:
glsl

// Sample normal from texture (in tangent space, range [0,1])
vec3 normal = texture(normalMap, fs_in.TexCoords).rgb;
...
// Now use this normal in your lighting calculations (Phong, PBR, etc.)
// It will work just like a regular surface normal

The catch: You need the TBN matrix

The tricky part is that you can't just use any normal — you need to pass the TBN matrix from your vertex shader to your fragment shader. This matrix is specific to each surface and tells the shader how to rotate the normal map vectors to align with that surface's orientation.

That's why the tutorial spends so much time on calculating tangent and bitangent vectors and constructing the TBN matrix. Without it, your normal map will look wrong when the surface is rotated or oriented differently.

So the short answer is: yes, sample and transform the normal, but make sure you have the TBN matrix ready first.
